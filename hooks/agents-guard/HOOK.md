...
actions:
  - body: >
      ...
      - If the request is architectural, cross-cutting, or scope is unclear: run
      \`codebase-indexer\` first, then read AGENTS.md.

      - If the request is narrowly scoped to known files/symbols: proceed with
      targeted discovery and avoid full indexing.


      When baseline refresh is needed:

      1. Invoke the \`codebase-indexer\` agent for this project (root:
      ${projectRoot}).
