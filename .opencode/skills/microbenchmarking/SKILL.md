...
**Adding the BenchmarkDotNet package**: Always use `dotnet add package BenchmarkDotNet` (no version) — this lets NuGet resolve the latest compatible version.
...
- **Move initialization to `[GlobalSetup]`** — setup inside the benchmark method is measured
- **Do not add manual loops** — BDN controls invocation count automatically
- **Mark a baseline** when comparing alternatives — use `[Benchmark(Baseline = true)]`
- **Store inputs in fields or `[Params]`**, not as literals or `const` values

### Step 3: Validate and run

1. Run with `--job Dry` first to catch compilation errors without spending time on measurement.
2. Run a single representative case with default settings to verify output looks correct.
